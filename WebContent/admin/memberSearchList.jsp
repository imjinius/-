			...
			<!-- Main -->
				<section id="main">
					...
					<article class="box post" style="font-size: 16px; margin: 0;">
													...
													<div class="table-wrapper">
														<table>
															...
															<c:forEach var="list" items="${searchList }">
																<tr>
																	<td><input type="checkbox" class="mems" name="mems" value="${list.id }"></td>
																	<td>${list.id }</td>
																	<td>${list.name }</td>
																	<td>${list.birth }</td>
																	<td>${list.tel }</td>
																	<td>${list.addr }</td>
																	<td>${list.w_count}개</td>
																	<td>${list.re_count }개</td>
																	<fmt:formatDate var="reg_date" value="${list.reg_date }" pattern="yyyy-MM-dd hh:mm:ss"/>
																	<td>${reg_date }</td>
																	<td><button class="button manage" onclick="updateCheck('${list.id}');">수정</button></td>
																</tr>
																</c:forEach>
															</thead>
														</table>
							...
							<span><button type="button" id="del" class="button manage" onclick="history.back();">뒤로가기</button></span>
